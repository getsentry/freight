...
class DeployLogApiView(ApiView, DeployMixin):
    ...
    def get(self, **kwargs):
        ...
        queryset = (
            ...
            .filter(LogChunk.task_id == deploy.task_id)
        ...
        if args.offset == -1:
            ...
            tail = (
                db.session.query(LogChunk.offset + LogChunk.size)
                .filter(LogChunk.task_id == deploy.task_id)
            ...
            if tail is None:
                ...
                if args.limit:
                    queryset = queryset.filter(
                        ...
                        >= max(tail - args.limit + 1, 0)
                    )
        else:
            if args.offset:
                queryset = queryset.filter(LogChunk.offset >= args.offset)
            if args.limit:
                queryset = queryset.filter(LogChunk.offset < args.offset + args.limit)
