...
class SlackNotifier(Notifier):
    ...
    def send_deploy(self, deploy, task, config, event):
        ...
        if event == NotifierEvent.TASK_QUEUED:
            ...
            title = "[{app_name}/{env}] Failed to deploy {user}'s <{link}|#{number}> ({sha}) after {duration}s".format(**params)
            ...
            title = "[{app_name}/{env}] Successfully deployed {user}'s <{link}|#{number}> ({sha}) after {duration}s".format(**params)
        else:
