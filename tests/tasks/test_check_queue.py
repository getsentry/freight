...
class CheckQueueTestCase(TransactionTestCase):
    ...
    def test_without_pending_task(self, mock_push):
        ...
        app = self.create_app(repository=repo)
        deploy_config = self.create_taskconfig(app=app)
        task = self.create_task(app=app, user=user, status=TaskStatus.in_progress)
        deploy = self.create_deploy(app=app, task=task)
        db.session.commit()
    ...
    def test_inprogress_pending_task(self, mock_push):
        ...
        task_1 = self.create_task(app=app_1, user=user, status=TaskStatus.in_progress)
        ...
        task_2 = self.create_task(app=app_2, user=user, status=TaskStatus.pending)
