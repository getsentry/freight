...
class DeployListTest(DeployIndexBase):
    ...
    def test_no_filters(self):
        ...
        deploy = self.create_deploy(app=self.app, task=task)
    ...
    def test_status_filter(self):
        task = self.create_task(app=self.app, user=self.user, status=TaskStatus.pending)
        deploy = self.create_deploy(app=self.app, task=task)
...
class DeployCreateTest(DeployIndexBase):
    ...
    def test_default_ref(self):
        resp = self.client.post(
            self.path,
            data={"env": "staging", "app": self.app.name, "user": self.user.name},
        )
        assert resp.status_code == 201
        data = json.loads(resp.data)
        assert data["id"]
    ...
    def test_current_ref(self):
        current = self.create_task(
            app=self.app, user=self.user, status=TaskStatus.finished, sha=uuid4().hex
        )
        self.create_deploy(app=self.app, task=current, environment="staging")
    ...
    def test_previous_ref_with_no_valids(self):
        ...
        self.create_deploy(app=self.app, task=task, environment="staging")
    ...
    def test_previous_ref(self):
        ...
        self.create_deploy(app=self.app, task=previous, environment="staging")
        current = self.create_task(
            app=self.app, user=self.user, status=TaskStatus.finished, sha=uuid4().hex
        )
        self.create_deploy(app=self.app, task=current, environment="staging")
